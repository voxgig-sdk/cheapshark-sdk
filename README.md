...
CheapShark is a price comparison website for digital PC Games, tracking prices across multiple stores including Steam, GreenManGaming, and Fanatical.

Learn more about CheapShark API at [cheapshark.com](https://cheapshark.com).
...
const items = await client.Alert().list({ email: "example" })
```
...
$alerts = $client->Alert()->list();
print_r(array_map(fn($item) => $item->data_get(), $alerts));
```
