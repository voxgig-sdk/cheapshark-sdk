...
class StoreEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = CheapsharkSDK::test(null, null);
        $ent = $testsdk->Store(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Store(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Store(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = store_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "store." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CHEAPSHARK_TEST_STORE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $store_ref01_data = null;
        if (count($store_ref01_data_raw) > 0) {
...
function store_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/store/StoreTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["store01", "store02", "store03"] as $k) {
    ...
    $entid_env_raw = getenv("CHEAPSHARK_TEST_STORE_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["CHEAPSHARK_TEST_STORE_ENTID"]);
