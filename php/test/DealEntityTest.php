...
class DealEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = CheapsharkSDK::test(null, null);
        $ent = $testsdk->Deal(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Deal(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Deal(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = deal_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "deal." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CHEAPSHARK_TEST_DEAL_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $deal_ref01_data = null;
        if (count($deal_ref01_data_raw) > 0) {
...
function deal_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/deal/DealTestData.json';
    ...
    $entid_env_raw = getenv("CHEAPSHARK_TEST_DEAL_ENTID");
