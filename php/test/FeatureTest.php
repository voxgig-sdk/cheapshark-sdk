...
class FtCtrl extends CheapsharkControl
{
...
class FtClient
{
    ...
    private CheapsharkUtility $utility;

    public function __construct(CheapsharkUtility $utility, array $options)
    {
    ...
    public function get_utility(): CheapsharkUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (CheapsharkContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public CheapsharkContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (CheapsharkContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = CheapsharkConfig::make_feature($name);
    ...
    private static function buildUrl(CheapsharkSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof CheapsharkError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new CheapsharkResponse($fetched);
        }
    ...
    private function fail(CheapsharkContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (CheapsharkContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
