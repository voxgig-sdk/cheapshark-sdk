...
class CheapsharkNetsimFeature extends CheapsharkBaseFeature
{
    ...
    private function _simulate(CheapsharkContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(CheapsharkContext $ctx, array $applied): void
    {
        $client = $this->client;
