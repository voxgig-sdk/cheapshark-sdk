...
local AlertEntity = {}
AlertEntity.__index = AlertEntity


function AlertEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AlertEntity)
  self._name = "alert"
  self._client = client
...
function AlertEntity:get_name()
  return self._name
end


function AlertEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AlertEntity.new(self._client, opts)
end
...
function AlertEntity:mark_deleted()
  self._deleted = true
end


function AlertEntity:deleted()
  return true == self._deleted
end


function AlertEntity:data_set(args)
  if args ~= nil then
...
function AlertEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AlertEntity:match_set(args)
  if args ~= nil then
...
function AlertEntity:match_get()
...
function AlertEntity:stream(action, args, callopts)
...
function AlertEntity:_run_op(ctx, post_done)
  local utility = self._utility
