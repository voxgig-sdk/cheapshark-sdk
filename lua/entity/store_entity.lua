...
local StoreEntity = {}
StoreEntity.__index = StoreEntity


function StoreEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, StoreEntity)
  self._name = "store"
  self._client = client
...
function StoreEntity:get_name()
  return self._name
end


function StoreEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return StoreEntity.new(self._client, opts)
end
...
function StoreEntity:mark_deleted()
  self._deleted = true
end


function StoreEntity:deleted()
  return true == self._deleted
end


function StoreEntity:data_set(args)
  if args ~= nil then
...
function StoreEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function StoreEntity:match_set(args)
  if args ~= nil then
...
function StoreEntity:match_get()
...
function StoreEntity:stream(action, args, callopts)
...
function StoreEntity:_run_op(ctx, post_done)
  local utility = self._utility
