...
local DealEntity = {}
DealEntity.__index = DealEntity


function DealEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DealEntity)
  self._name = "deal"
  self._client = client
...
function DealEntity:get_name()
  return self._name
end


function DealEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DealEntity.new(self._client, opts)
end
...
function DealEntity:mark_deleted()
  self._deleted = true
end


function DealEntity:deleted()
  return true == self._deleted
end


function DealEntity:data_set(args)
  if args ~= nil then
...
function DealEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DealEntity:match_set(args)
  if args ~= nil then
...
function DealEntity:stream(action, args, callopts)
...
function DealEntity:_run_op(ctx, post_done)
  local utility = self._utility
