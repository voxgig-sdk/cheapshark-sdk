...
describe("DealEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set CHEAPSHARK_TEST_DEAL_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local deal_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.deal")))
    local deal_ref01_data = nil
    if #deal_ref01_data_raw > 0 then
      deal_ref01_data = helpers.to_map(deal_ref01_data_raw[1][2])
    end
...
function deal_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("CHEAPSHARK_TEST_DEAL_ENTID")
