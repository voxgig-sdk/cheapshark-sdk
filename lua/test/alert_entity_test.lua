...
describe("AlertEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:Alert(nil)
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Alert(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Alert(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set CHEAPSHARK_TEST_ALERT_ENTID JSON to run live")
      return
    end
...
function alert_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/alert/AlertTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read alert test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("CHEAPSHARK_TEST_ALERT_ENTID")
