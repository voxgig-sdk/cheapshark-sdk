...
The Lua SDK for the Cheapshark API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Alert(nil):list(nil, nil)
...
```lua
-- Create
local created, _ = client:Alert(nil):create({ name = "Example" }, nil)
...
local result, err = client:Cheapshark(nil):load(
  { id = "test01" }, nil
)
...
```
lua/
├── cheapshark_sdk.lua    -- Main SDK module
...
The main module (`cheapshark_sdk`) exports the SDK constructor
