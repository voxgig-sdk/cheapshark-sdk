...
boru-driven command-line client **and** interactive REPL for the Cheapshark
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/cheapshark-cli)
make build
...
./cheapshark-cli list deal

# 5. Override the API base URL for a single call
CHEAPSHARK_BASE=https://api.example.com ./cheapshark-cli list alert

# 6. No arguments -> interactive REPL
./cheapshark-cli
cheapshark> list alert
cheapshark> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/cheapshark-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/cheapshark-cli list alert
   ```
...
Run with no arguments to open a REPL (prompt `cheapshark>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `alert`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/cheapshark-cli`. |
...
(`./cheapshark-cli <expr>`) and interactively (the REPL), and expressions compose the
