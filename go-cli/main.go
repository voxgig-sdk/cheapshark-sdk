// Command Cheapshark-cli is a CLI + REPL for the Cheapshark SDK,
...
const prompt = "cheapshark"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "alert deal game store"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      CHEAPSHARK_APIKEY (api key), CHEAPSHARK_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: CHEAPSHARK_APIKEY carries the API key and
	// CHEAPSHARK_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("CHEAPSHARK_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("CHEAPSHARK_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewCheapsharkSDK(opts)
