...
class CheapsharkTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            CheapsharkTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = CheapsharkTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = CheapsharkTestRunner.getenv("CHEAPSHARK_TEST_LIVE")
        override = CheapsharkTestRunner.getenv("CHEAPSHARK_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = CheapsharkTestRunner.getenv(key)
        ...
        explain = CheapsharkTestRunner.getenv("CHEAPSHARK_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["CHEAPSHARK_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if CheapsharkTestRunner._test_control is not None:
            return CheapsharkTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                CheapsharkTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            CheapsharkTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = CheapsharkTestRunner.load_test_control()
...
def load_env_local():
    CheapsharkTestRunner.load_env_local()


def env_override(m):
    return CheapsharkTestRunner.env_override(m)


def entity_data(v):
    return CheapsharkTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return CheapsharkTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return CheapsharkTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return CheapsharkTestRunner.load_test_control()


def live_client_options():
    return CheapsharkTestRunner.live_client_options()


def live_delay_ms():
    return CheapsharkTestRunner.live_delay_ms()
