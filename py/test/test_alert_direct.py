...
class TestAlertDirect:

    def test_should_direct_list_alert(self):
        setup = _alert_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-alert", "live" if setup["live"] else "unit")
        if _skip:
...
def _alert_direct_setup(mockres):
