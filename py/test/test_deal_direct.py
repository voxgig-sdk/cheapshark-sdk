...
class TestDealDirect:

    def test_should_direct_list_deal(self):
        setup = _deal_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-deal", "live" if setup["live"] else "unit")
        if _skip:
...
def _deal_direct_setup(mockres):
