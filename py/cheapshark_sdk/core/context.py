...
from cheapshark_sdk.core.helpers import get_ctx_prop, to_map
...
class CheapsharkContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        if isinstance(sp, CheapsharkSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, CheapsharkResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, CheapsharkResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def make_error(self, code, msg):
        return CheapsharkError(code, msg, self)
