...
class DealEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return DealEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Deal]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Deal().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
