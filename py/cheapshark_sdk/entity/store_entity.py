...
from cheapshark_sdk.cheapshark_types import (
    Store,
    StoreListMatch,
)
...
class StoreEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return StoreEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Store]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Store().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
