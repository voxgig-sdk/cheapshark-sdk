...
//     (\`new CheapsharkSDK(...)\` / \`CheapsharkSDK.test(...)\` become
//     \`CheapsharkSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"alert":{"test01":{"id":"test01"}},"deal":{"test01":{"id":"test01"}},"game":{"test01":{"id":"test01"}},"store":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new CheapsharkSDK(...)\` and any
// \`CheapsharkSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+CheapsharkSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/CheapsharkSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
