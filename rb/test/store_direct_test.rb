...
class StoreDirectTest < Minitest::Test
  def test_direct_list_store
    setup = store_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-store", setup[:live] ? "live" : "unit")
    if _should_skip
...
def store_direct_setup(mockres)
