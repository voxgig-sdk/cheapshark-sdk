...
class StoreEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.Store(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Store(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set CHEAPSHARK_TEST_STORE_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def store_basic_setup(extra)
  ...
  entid_env_raw = ENV["CHEAPSHARK_TEST_STORE_ENTID"]
