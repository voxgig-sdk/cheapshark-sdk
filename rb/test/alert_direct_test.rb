...
class AlertDirectTest < Minitest::Test
  def test_direct_list_alert
    setup = alert_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-alert", setup[:live] ? "live" : "unit")
    if _should_skip
...
def alert_direct_setup(mockres)
