...
require_relative 'Cheapshark_types'
...
class CheapsharkSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = CheapsharkHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = CheapsharkHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, CheapsharkFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    CheapsharkUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = CheapsharkHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = CheapsharkHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = CheapsharkHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => CheapsharkError.new(
        "#{op}_allow",
        "CheapsharkSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = CheapsharkHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = CheapsharkError.new(
        "graphql_error", "CheapsharkSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
