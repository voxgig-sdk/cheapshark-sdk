...
class StoreEntity
  ...
  def make
    opts = @_entopts.dup
    StoreEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = CheapsharkHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Store, Hash] the current Store data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = CheapsharkHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Store fields)
  def match_get
  ...
  #
  # @param reqmatch [StoreListMatch, Hash, nil] match filter (any subset of
  #   Store fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Store>, Array] the matching Store items; raises CheapsharkError on failure
