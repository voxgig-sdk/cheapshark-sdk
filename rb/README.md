...
raise a `CheapsharkError` (a `StandardError` subclass), so wrap
...
| --- | --- | --- |
| `images` | `Hash` |  |
| `isActive` | `Integer` | Whether the store is active (0 or 1) |
...
```
rb/
├── Cheapshark_sdk.rb       -- Main SDK module
