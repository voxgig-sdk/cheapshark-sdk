...
func TestStoreEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set CHEAPSHARK_TEST_STORE_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		storeRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.store")))
...
func storeBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("CHEAPSHARK_TEST_STORE_ENTID")
