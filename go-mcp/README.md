...
[MCP](https://modelcontextprotocol.io) server exposing the Cheapshark SDK as
two agent tools — `cheapshark_list` and `cheapshark_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/cheapshark-mcp)
make build

# 2. Provide credentials via the environment
export CHEAPSHARK_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user cheapshark \
  -- /absolute/path/to/cheapshark-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./cheapshark-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// cheapshark_list: first page of records
{ "entity": "alert" }
{ "entity": "alert", "query": { } }

// cheapshark_load: one record by id
{ "entity": "alert", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/cheapshark-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user cheapshark \
     -- "$PWD"/dist/*/cheapshark-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `cheapshark_list` and `cheapshark_load` tools now appear
   in new sessions. Ask the agent to *"list alert using cheapshark"*
   and it calls `cheapshark_list` with `{"entity":"alert"}`.
...
```sh
export CHEAPSHARK_APIKEY=sk_live_xxx            # API key
export CHEAPSHARK_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `cheapshark_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `cheapshark_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `CHEAPSHARK_APIKEY` | API key sent with every request. |
| `CHEAPSHARK_BASE` | Optional override of the API base URL. |

### Entities

The 4 entities valid as the `entity` argument:

alert | deal | game | store

### Smoke test via HTTP (raw JSON-RPC)

```sh
./cheapshark-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"cheapshark_load","arguments":{"entity":"alert","query":{"id":1}}}}'
```
