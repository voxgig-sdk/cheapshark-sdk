...
{
    private static function utility(): CheapsharkUtility
    {
        return new CheapsharkUtility();
    }
    ...
    private static function ctx(array $over = []): CheapsharkContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new CheapsharkContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new CheapsharkOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof CheapsharkError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new CheapsharkOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = CheapsharkMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new CheapsharkOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = CheapsharkMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new CheapsharkOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = CheapsharkMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new CheapsharkSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = CheapsharkMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new CheapsharkResponse([]),
            'result' => new CheapsharkResult([]),
        ]);
        [, $err] = CheapsharkMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new CheapsharkSpec([]),
            'response' => null,
            'result' => new CheapsharkResult([]),
        ]);
        [, $err] = CheapsharkMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new CheapsharkSpec(['step' => 's']),
            'response' => new CheapsharkResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new CheapsharkResult([]),
        ]);
        [, $err] = CheapsharkMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new CheapsharkSpec(['step' => 's']),
            'response' => new CheapsharkResponse(self::resp(200, ['v' => 1])),
            'result' => new CheapsharkResult([]),
        ]);
        [, $err] = CheapsharkMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new CheapsharkSpec(['step' => 's']),
            'response' => new CheapsharkResponse(self::resp(200, ['v' => 2])),
            'result' => new CheapsharkResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        CheapsharkMakeResponse::call($ctx);
    ...
    {
        $preset = new CheapsharkResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new CheapsharkSpec([]),
            'response' => new CheapsharkResponse([]),
            'result' => new CheapsharkResult([]),
        ...
        [$got, $err] = CheapsharkMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new CheapsharkResult([])]);
        [, $err] = CheapsharkMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new CheapsharkSpec([]), 'result' => null]);
        [, $err] = CheapsharkMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new CheapsharkSpec(['step' => 's']),
            'result' => new CheapsharkResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new CheapsharkOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = CheapsharkMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new CheapsharkSpec(['step' => 's']),
            'result' => new CheapsharkResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new CheapsharkOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = CheapsharkMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new CheapsharkResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new CheapsharkSpec([]),
            'result' => new CheapsharkResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = CheapsharkMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = CheapsharkMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new CheapsharkError('boom', 'boom');
        $utility->fetcher = function (CheapsharkContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new CheapsharkSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = CheapsharkMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (CheapsharkContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = CheapsharkMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(CheapsharkResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        CheapsharkMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (CheapsharkContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new CheapsharkSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = CheapsharkMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new CheapsharkResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new CheapsharkSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = CheapsharkMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = CheapsharkMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = CheapsharkMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new CheapsharkResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, CheapsharkDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new CheapsharkResult(['ok' => false])]);
        $this->expectException(CheapsharkError::class);
        CheapsharkDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new CheapsharkResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', CheapsharkMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        CheapsharkMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new CheapsharkBaseFeature();
        $b = new CheapsharkBaseFeature();
        CheapsharkFeatureAdd::call($ctx, $a);
        CheapsharkFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): CheapsharkBaseFeature
    {
        $f = new CheapsharkBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        CheapsharkFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): CheapsharkContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new CheapsharkSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = CheapsharkPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        CheapsharkPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new CheapsharkResponse(['headers' => null]),
            'result' => new CheapsharkResult([]),
        ]);
        CheapsharkResultHeaders::call($ctx);
