...
class CheapsharkTestFeature extends CheapsharkBaseFeature
{
    ...
    public function init(CheapsharkContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (CheapsharkContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = CheapsharkParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (CheapsharkContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(CheapsharkContext $ctx, $op, $args): array
    {
