...
module CheapsharkUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = CheapsharkResponse.new({})
    ...
    if fetch_err
      ...
      response = CheapsharkResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = CheapsharkResponse.new(fetched)
    else
